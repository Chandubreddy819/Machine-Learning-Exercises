...
def process_and_train(labels, train, num_images, h, w):
    ...
    eigen_faces = calculate_eigenfaces(train, avg_face, num_eigenfaces)

    # calculate the coefficients/features for all images --> get_feature_representation()
    features = get_feature_representation(train, eigen_faces, avg_face, num_eigenfaces)

    # train the classifier using the calculated features
    clf.fit(features, labels)

    return eigen_faces, num_eigenfaces, avg_face
...
def calculate_average_face(train):
    ...
    '''
    avg_image = np.mean(train, 0)  # calculated the mean along the column
    return avg_image
...
def calculate_eigenfaces(train, avg, num_eigenfaces):
    ...
    # plt.imshow(eigenfaces[0].reshape(50, 37), cmap='gray')
    # plt.axis('off')
    # plt.show()
    return eigenfaces
...
def get_feature_representation(images, eigenfaces, avg, num_eigenfaces):
    ...
    # Ensure images is a 2D array
    if len(images.shape) == 1:
        images = np.array([images])

    coefficients = np.zeros((images.shape[0], num_eigenfaces))
    selected_eigenfaces = eigenfaces[:num_eigenfaces]

    for i in range(images.shape[0]):
        centered_images = images[i] - avg
        coefficients[i, :] = np.dot(selected_eigenfaces, centered_images)
...
def reconstruct_image(img, eigenfaces, avg, num_eigenfaces, h, w):
    ...
    coefficients = get_feature_representation(img_vector, eigenfaces, avg, num_eigenfaces)
    ...
    for i in range(num_eigenfaces):
        reconstructed_img += coefficients[i] * eigenfaces[i]
...
def classify_image(img, eigenfaces, avg, num_eigenfaces, h, w):
    ...
    predicted_label = clf.predict(coefficients.reshape(1, -1))
